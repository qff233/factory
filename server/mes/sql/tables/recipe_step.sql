CREATE TABLE mes.recipe_steps (
    id SERIAL PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    recipe_id INT NOT NULL REFERENCES mes.recipes(id),
    quantity INT NOT NULL,
    step_index INT NOT NULL,
    step_description TEXT,
    created_by VARCHAR(50) NOT NULL
);
CREATE INDEX idx_recipe_steps_name ON mes.recipe_steps(name);

CREATE OR REPLACE FUNCTION mes.check_recipe_active()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM mes.recipes
        WHERE id = NEW.recipe_id AND status = 'active'
    ) THEN
        RAISE EXCEPTION '只能引用状态为active的配方';
    END IF;
    return NEW;
END;
$$ LANGUAGE 'plpgsql';

CREATE TRIGGER ensure_active_recipe
    BEFORE INSERT OR UPDATE ON mes.recipe_steps
    FOR EACH ROW
    EXECUTE FUNCTION mes.check_recipe_active();

---- TEST -----

-- panic!
WITH recipe AS (
    SELECT id FROM mes.recipes
    WHERE name = '水' AND status = 'inactive'
    LIMIT 1
)
INSERT INTO mes.recipe_steps(
    name,
    recipe_id,
    quantity,
    step_index,
    created_by
)
SELECT '随便', id, 1, 1,'EC2' FROM recipe;

WITH recipe AS (
    SELECT id FROM mes.recipes
    WHERE name = '水' AND status = 'active'
)
INSERT INTO mes.recipe_steps(
    name,
    recipe_id,
    quantity,
    step_index,
    created_by
)
SELECT '随便', id, 1, 1,'EC2' FROM recipe;

SELECT * FROM mes.recipe_steps;
